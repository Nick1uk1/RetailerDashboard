async function testFullOrder() {
  // Authenticate
  const authParams = new URLSearchParams({
    applicationId: '51e2b5ee-bb88-4a20-b0d6-72b74f3e9642',
    applicationSecret: '5daa226d-bb00-43f9-b26c-dfb755323246',
    token: '1b385b6dee2b50f8717b0520d06f266e',
  });

  const authRes = await fetch('https://api.linnworks.net/api/Auth/AuthorizeByApplication', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: authParams.toString(),
  });

  const auth = await authRes.json();
  console.log('Server:', auth.Server);

  // Create order matching our portal payload
  const order = [{
    Source: 'MANUAL',
    SubSource: 'HOMECOOKS-HQ',
    ReferenceNumber: 'TEST-FULL-' + Date.now(),
    ExternalReference: 'TEST-FULL-' + Date.now(),
    ReceivedDate: new Date().toISOString(),
    DispatchBy: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000).toISOString(),
    Currency: 'GBP',
    ChannelBuyerName: 'HomeCooks HQ',
    PaymentStatus: 'UNPAID',
    PaidAmount: 0,
    DeliveryAddress: {
      FullName: 'HomeCooks HQ',
      Company: 'HomeCooks HQ',
      Address1: '123 Test Street',
      Town: 'London',
      PostCode: 'SW1A 1AA',
      Country: 'United Kingdom',
      EmailAddress: 'nick@homecooks.app',
    },
    BillingAddress: {
      FullName: 'HomeCooks HQ',
      Company: 'HomeCooks HQ',
      Address1: '123 Test Street',
      Town: 'London',
      PostCode: 'SW1A 1AA',
      Country: 'United Kingdom',
      EmailAddress: 'nick@homecooks.app',
    },
    OrderItems: [{
      ItemNumber: 'A00735.',
      ChannelSKU: 'A00735.',
      ItemTitle: 'Porcini Glazed Lithuanian Chicken - Chef Romas (Case x 6)',
      Qty: 6,
      PricePerUnit: 3.90,
      TaxRate: 20,
      TaxCostInclusive: true,
      LinePercentDiscount: 0,
    }]
  }];

  console.log('Sending order...');

  const orderParams = new URLSearchParams({
    orders: JSON.stringify(order),
  });

  const orderRes = await fetch(auth.Server + '/api/Orders/CreateOrders', {
    method: 'POST',
    headers: {
      'Authorization': auth.Token,
      'Content-Type': 'application/x-www-form-urlencoded',
    },
    body: orderParams.toString(),
  });

  console.log('Response status:', orderRes.status);
  const result = await orderRes.text();
  console.log('Response body:', result);

  // Check if order was created
  if (orderRes.status === 200) {
    const ids = JSON.parse(result);
    if (ids.length > 0) {
      console.log('\nChecking if order exists...');

      // Wait a moment
      await new Promise(r => setTimeout(r, 2000));

      const checkRes = await fetch(auth.Server + '/api/Orders/GetOrdersById', {
        method: 'POST',
        headers: {
          'Authorization': auth.Token,
          'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: new URLSearchParams({
          pkOrderIds: JSON.stringify(ids),
        }).toString(),
      });

      const orders = await checkRes.json();
      console.log('Orders found:', orders.length);
      if (orders.length > 0) {
        console.log('Order ID:', orders[0].OrderId);
        console.log('NumOrderId:', orders[0].NumOrderId);
        console.log('IsParked:', orders[0].GeneralInfo?.IsParked);
        console.log('Source:', orders[0].GeneralInfo?.Source);
      }
    }
  }
}

testFullOrder().catch(console.error);
